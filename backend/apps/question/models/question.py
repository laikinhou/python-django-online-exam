...
class TblQuestion(models.Model):
    ...
    option_ids = models.JSONField(null=True, blank=True, default=None, help_text='题目的选项是由选项ID组成的数组')
    anser_ids = models.JSONField(null=True, blank=True, default=None, help_text='题目的答案是由选项ID组成的数组')
    create_time = models.DateTimeField(auto_now_add=True, help_text='创建时间')
    update_time = models.DateTimeField(auto_now=True, help_text='更新时间')
    
    creator = models.ForeignKey(UserModel, on_delete=models.DO_NOTHING, db_constraint=False, help_text='题目创建者', blank=True)
